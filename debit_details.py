...
class AddDebitedTransactionScreen(Screen):
    def __init__(self, **kwargs):
        super(AddDebitedTransactionScreen, self).__init__(**kwargs)
        self.load_ui()
    ...
    def load_ui(self):
        ...
        self.category_spinner = Spinner(
            ...
            values=["TV Recharge","Groceries","Clothes","Cylinder Booking","ATM Withdrawal","Repair","Food","Travel","Other","LIC"],
            size_hint=(1, 0.2)
        )
    ...
    def save_transaction(self, instance):
        ...
        print(bank_row,det[0],det[2])
        if df_banks.loc[bank_row,"Balance"].values[0]<float(amount):
            self.show_low_balance_popup(df_banks.loc[bank_row,"Balance"].values[0],amount)
            self.clear_inputs()
        else:
        # Create DataFrame to store the transaction
            ...
            df_banks = pd.read_csv('bank_details.csv')
            bank_row = df_banks.loc[df_banks['Bank Name']==det[0]].index
            print(bank_row, "nsvbjhbasjdvg")
            if not bank_row.empty:
                ...
                if category=='ATM Withdrawal':
                    df_banks.loc[df_banks['Bank Name']=='Cash','Balance']=df_banks.loc[df_banks['Bank Name']=='Cash','Balance']+float(amount)


                # Save the updated data back to the CSV
                df_banks.to_csv("bank_details.csv", index=False)
                self.show_balance_popup(selected_bank, df_banks.loc[bank_row, 'Balance'].values[0],category,amount)
    ...
    def show_balance_popup(self, selected_bank, updated_balance,category,amount):
        if updated_balance is not None:
            # Create a message for the popup
            content = f"Balance updated for \n{selected_bank.split(' ')[0]}: \n New Balance = {updated_balance}"
            if category=='ATM Withdrawal':
                content+=f"and\nCash has been added with {amount}"
            ...
            popup.open()
    def show_low_balance_popup(self, balance, amount):

            # Create a message for the popup
            content = f"Current Balance {balance} is less then {amount}"
